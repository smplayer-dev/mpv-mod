...
#define OPT_BASE_STRUCT struct shm_opts

struct shm_opts {
	char * buffer_name;
};

static const struct m_sub_options shm_conf = {
    .opts = (const struct m_option[]) {
       {"buffer-name", OPT_STRING(buffer_name)},
       {0}
    },
    .defaults = &(const struct shm_opts) { .buffer_name = "mpv", },
    .size = sizeof(struct shm_opts),
};


struct priv {
	//char * buffer_name;
	struct shm_opts * opts;
};
...
static int preinit(struct vo *vo)
{
	...
	p->opts = mp_get_config_group(vo, vo->global, &shm_conf);
/*
	struct priv * p = vo->priv;
...
/*
    ...
    .defaults = &(const OPT_BASE_STRUCT) { .buffer_name = "mpv", },
*/
    .global_opts = &shm_conf,
    .options_prefix = "shm",
};
